...
exports.getCuisineItems = getCuisineItems;
exports.getCuisineItemBySlug = getCuisineItemBySlug;
...
async function getCircuitBySlug(req, res, next) {
    try {
        ...
        const districtNames = Array.from(new Set(circuit.destinations
...
async function getCuisineItems(req, res, next) {
    try {
...
async function getCuisineItemBySlug(req, res, next) {
    try {
        const { slug } = req.params;
        ...
        if (!item) {
            throw new apiError_js_1.ApiError(404, 'Cuisine item not found');
        }
        const formatted = {
            ...item,
            restaurants: parseJsonField(item.restaurants, [])
