      ...
      <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex justify-end transition-opacity duration-300">
        <div className="bg-cream w-full max-w-xl h-full shadow-2xl overflow-y-auto flex flex-col p-6 md:p-8 border-l border-brand-brown/20">
          <div className="flex items-center justify-between border-b border-brand-brown/15 pb-4 mb-6">
            <h3 className="font-serif text-2xl text-brand-black"><LocalizedText text={"SETU AI Companion"} /></h3>
          ...
          <div className="flex-grow flex flex-col items-center justify-center text-center space-y-4">
            ...
            <p className="font-serif text-brand-black"><LocalizedText text={"Sign in to start planning your Bihar journey with the SETU AI Companion."} /></p>
            <Link
              to="/login"
              onClick={onClose}
      ...
      <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex justify-end transition-opacity duration-300">
        <div className="bg-cream w-full max-w-xl h-full shadow-2xl overflow-y-auto flex flex-col p-6 md:p-8 border-l border-brand-brown/20">
          ...
          <div className="flex-grow flex flex-col items-center justify-center text-center space-y-3">
            <p className="font-serif text-brand-black"><LocalizedText text={"The SETU AI Companion is available to customer accounts."} /></p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex justify-end transition-opacity duration-300">
      <div className="bg-cream w-full max-w-xl h-full shadow-2xl overflow-y-auto flex flex-col justify-between p-6 md:p-8 border-l border-brand-brown/20">
        ...
        <div>
          <div className="flex items-center justify-between border-b border-brand-brown/15 pb-4 mb-6">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h3 className="font-serif text-2xl text-brand-black"><LocalizedText text={"SETU AI Companion"} /></h3>
                <p className="text-xs font-sans text-brand-brown/70"><LocalizedText text={"Personalized Bihar Concierge & Itinerary Planner"} /></p>
              </div>
            </div>
            <button onClick={onClose} className="p-2 text-brand-black/60 hover:text-brand-black">
              <X className="w-6 h-6" />
          ...
          <form onSubmit={handleGenerate} className="space-y-6">
            <div>
              <label className="block text-xs sub-nav-label text-brand-maroon mb-2"><LocalizedText text={" 1. SELECT YOUR TRAVEL INTERESTS "} /></label>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-xs sub-nav-label text-brand-maroon mb-2"><LocalizedText text={" 2. DURATION (DAYS) "} /></label>
                <input
                  type="number"
                  min={1}
                  max={14}
                ...
                />
                {!user.isPremium && (
                  <p className="text-[10px] font-sans text-brand-brown/60 mt-1"><LocalizedText text={"Free plan generates a 1-day preview. SETU Plus unlocks your full "} />{duration}<LocalizedText text={"-day itinerary."} /></p>
                )}
              </div>

              <div>
                <label className="block text-xs sub-nav-label text-brand-maroon mb-2"><LocalizedText text={" 3. STARTING CITY "} /></label>
                <select
                  value={startingCity}
                ...
                >
                  ...
                  <option value="Bodh Gaya"><LocalizedText text={"Bodh Gaya"} /></option>
                  <option value="Darbhanga"><LocalizedText text={"Darbhanga"} /></option>
                  <option value="Gaya Ji"><LocalizedText text={"Gaya Ji"} /></option>
                </select>
              </div>
            </div>
            ...
            >
              {loading ? (
                <span><LocalizedText text={"CURATING ITINERARY..."} /></span>
              ) : (
                <>
                  <Send className="w-4 h-4" />
                  <span><LocalizedText text={"CURATE MY BIHAR JOURNEY"} /></span>
                </>
              )}
            </button>
          </form>
          ...
          {result && (
            <div className="mt-8 pt-6 border-t border-brand-brown/20 space-y-6">
              <div className="bg-white p-5 rounded border border-brand-gold/30 shadow-sm">
                <span className="text-[10px] sub-nav-label text-brand-gold uppercase tracking-widest"><LocalizedText text={" AI SUGGESTED JOURNEY "} /></span>
              ...
              <div className="space-y-4">
                <h5 className="sub-nav-label text-brand-maroon"><LocalizedText text={"DAY-BY-DAY ITINERARY"} /></h5>
                {result.dayByDayItinerary.map((day) => (
                  <div key={day.day} className="bg-white p-4 rounded border border-brand-brown/10 space-y-2">
                    <div className="flex items-center justify-between text-xs font-bold text-brand-black font-serif">
                      <span><LocalizedText text={"DAY "} />{day.day}: {day.title}</span>
                    </div>
                    ...
                    {day.recommendedFood && (
                      <div className="pt-2 border-t border-brand-brown/10 flex items-center space-x-2 text-xs font-sans text-brand-maroon">
                        ...
                        <span><LocalizedText text={"Food Tip: "} />{day.recommendedFood}</span>
                      </div>
                    )}
                  </div>
                ))}
              ...
              {tier === 'free' && (
                <div className="bg-brand-maroon/10 border border-brand-gold/40 p-4 rounded space-y-3 text-center">
                  <p className="text-xs font-serif text-brand-black"><LocalizedText text={" This is a 1-day preview. Upgrade to "} /><strong><LocalizedText text={"SETU Plus"} /></strong><LocalizedText text={" for your full multi-day itinerary, local insider tips, and food recommendations. "} /></p>
                  <Link
                    to="/account"
                    onClick={onClose}
                  ...
                  ><LocalizedText text={" UPGRADE TO SETU PLUS — ₹99/MO "} /></Link>
                </div>
              )}
              ...
              {result.insiderTips && result.insiderTips.length > 0 && (
                <div className="bg-brand-black text-cream p-4 rounded space-y-2 text-xs font-serif">
                  <span className="sub-nav-label text-brand-gold text-[10px]"><LocalizedText text={"LOCAL INSIDER TIPS"} /></span>
                  <ul className="space-y-1">
                    {result.insiderTips.map((tip, idx) => (
                      <li key={idx}><LocalizedText text={"• "} />{tip}</li>
                    ))}
                  </ul>
                </div>
              )}
        ...
        <div className="mt-8 pt-4 border-t border-brand-brown/15 text-center text-xs font-sans text-brand-brown/60"><LocalizedText text={" Powered by Google Gemini 2.5 & SETU Bihar Tourism Data "} /></div>
