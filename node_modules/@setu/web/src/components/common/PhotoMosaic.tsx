...
export const PhotoMosaic: React.FC<PhotoMosaicProps> = ({
  ...
  if (!images || images.length === 0) {
    return (
      <div className="bg-cream/50 p-8 text-center rounded-xl border border-brand-brown/15 text-xs sub-nav-label text-brand-brown/70"><LocalizedText text={" NO GALLERY IMAGES AVAILABLE "} /></div>
    );
  }
      ...
      {images.map((imgUrl, idx) => (
        ...
        >
          ...
          <div className="absolute inset-0 bg-black/45 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col items-center justify-center space-y-2 text-white p-4">
            ...
            <span className="text-[10px] sub-nav-label tracking-widest text-brand-gold font-semibold uppercase"><LocalizedText text={" ENLARGE PHOTO "} /></span>
          </div>
        </div>
      ))}
    </div>
