    ...
    <div className="pt-24 pb-20 px-4 md:px-8 max-w-[1500px] mx-auto space-y-6">
      ...
      <div className="bg-cream/90 backdrop-blur-md p-6 rounded-xl border border-brand-brown/15 shadow-sm space-y-2">
        <div className="flex items-center space-x-2 text-brand-maroon">
          ...
          <span className="sub-nav-label tracking-widest text-xs font-semibold"><LocalizedText text={"URBAN INTELLIGENCE MATRIX"} /></span>
        </div>
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h1 className="text-3xl md:text-5xl font-serif text-brand-black"><LocalizedText text={"Bihar City Hub Explorer"} /></h1>
            <p className="text-sm font-serif text-brand-black/75 mt-1 max-w-3xl leading-relaxed"><LocalizedText text={" Comprehensive 8-location urban matrix featuring tourism scores, healthcare rating, hospitality infrastructure, business connectivity, and individual place-level map navigation. "} /></p>
          </div>
          ...
          <div className="flex items-center space-x-3 bg-white/80 p-3 rounded-lg border border-brand-brown/20 text-xs text-brand-black">
            ...
            <div>
              <div className="font-bold"><LocalizedText text={"8 Dedicated City Hubs"} /></div>
              <div className="text-brand-brown text-[11px]"><LocalizedText text={"Individual Amenity Map Markers"} /></div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-brand-black text-white p-4 md:p-5 rounded-xl shadow-lg grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 items-center">
        ...
        <div className="space-y-1">
          <label className="text-[11px] sub-nav-label text-brand-gold tracking-wider block"><LocalizedText text={" LOCATION HUB "} /></label>
          <select
          ...
          >
            <option value="ALL" className="bg-brand-black text-white"><LocalizedText text={"All 8 City Hubs"} /></option>
            {hubs.map(hub => (
        ...
        <div className="space-y-1">
          <label className="text-[11px] sub-nav-label text-brand-gold tracking-wider block"><LocalizedText text={" FILTER CATEGORY "} /></label>
          <select
            value={selectedCategory}
            onChange={(e) => setSelectedCategory(e.target.value)}
            className="w-full bg-white/10 border border-white/20 rounded px-3 py-2 text-xs text-cream font-medium focus:outline-none focus:border-brand-gold transition-colors"
          >
            <option value="ALL" className="bg-brand-black text-white"><LocalizedText text={"All Categories (Places, Medical, Hotels)"} /></option>
            <option value="Tourist Place" className="bg-brand-black text-white"><LocalizedText text={"Tourist Places & Heritage"} /></option>
            <option value="Hospital" className="bg-brand-black text-white"><LocalizedText text={"Hospitals & Healthcare"} /></option>
            <option value="Hotel" className="bg-brand-black text-white"><LocalizedText text={"Hotels & Stays"} /></option>
            <option value="Temple" className="bg-brand-black text-white"><LocalizedText text={"Temples & Holy Sites"} /></option>
          </select>
        </div>

        {/* SEARCH BOX */}
        <div className="space-y-1">
          <label className="text-[11px] sub-nav-label text-brand-gold tracking-wider block"><LocalizedText text={" SEARCH PLACES & HUBS "} /></label>
        ...
        <div className="flex items-center justify-between sm:justify-end space-x-3 pt-2 sm:pt-0">
          <div className="text-right">
            <div className="text-xs font-semibold text-brand-gold">{filteredHubs.length}<LocalizedText text={" Hubs Visible"} /></div>
          ...
          {(selectedHubSlug !== 'ALL' || selectedCategory !== 'ALL' || searchQuery !== '' || selectedPlace !== null) && (
            ...
            >
              <X className="w-3.5 h-3.5" />
              <span><LocalizedText text={"Reset"} /></span>
            </button>
          )}
        </div>
      </div>
      ...
      {loading ? (
        <div className="py-24 text-center text-brand-brown font-serif text-lg"><LocalizedText text={" Loading City Hub Explorer dataset... "} /></div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
          ...
          <div className={`${activeHub ? 'lg:col-span-7' : 'lg:col-span-12'} transition-all duration-300 relative rounded-xl overflow-hidden border border-brand-brown/20 shadow-md h-[400px] sm:h-[500px] lg:h-[680px]`}>
                  ...
                  >
                    <Popup className="city-hub-popup">
                      <div className="p-1 space-y-2 max-w-xs font-sans">
                        ...
                        >
                          <span><LocalizedText text={"Open City Hub Details"} /></span>
                    ...
                    <div className="p-1.5 space-y-2 max-w-xs font-sans">
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <span className="text-[10px] text-brand-brown font-mono font-bold">
                          {activeHub?.name}<LocalizedText text={" HUB "} /></span>
                      </div>
                      ...
                      <div className="text-[11px] text-brand-brown/80 flex items-center space-x-1.5 pt-1.5 border-t border-slate-100">
                        ...
                        <span><LocalizedText text={"Coordinates: ("} />{selectedPlace.latitude.toFixed(4)}, {selectedPlace.longitude.toFixed(4)})</span>
                      </div>
                    </div>
                  </Popup>
                </Marker>
            ...
            <div className="absolute top-3 left-3 right-3 z-[1000] pointer-events-none flex items-center justify-between">
              <div className="pointer-events-auto bg-brand-black/90 backdrop-blur-md text-white px-3 py-1.5 rounded-full text-xs font-serif flex items-center space-x-2 border border-white/20 shadow-md">
                ...
                <span><LocalizedText text={"Showing 8 Verified Bihar City Hubs"} /></span>
              </div>
            </div>
          </div>
          ...
          {activeHub ? (
              ...
              <div className="relative h-48 w-full bg-brand-black text-white shrink-0">
                ...
                <div className="absolute bottom-4 left-4 right-4 space-y-1">
                  <span className="text-[10px] sub-nav-label text-brand-gold uppercase tracking-widest block">
                    {activeHub.region}<LocalizedText text={" REGION "} /></span>
              ...
              <div className="p-5 overflow-y-auto space-y-5 flex-1 custom-scrollbar">
                ...
                <div className="bg-amber-50/80 border border-amber-200 p-3.5 rounded-lg space-y-1">
                  <div className="flex items-center space-x-1.5 text-xs sub-nav-label text-amber-800 font-semibold">
                    ...
                    <span><LocalizedText text={"KEY VERDICT & ASSESSMENT"} /></span>
                  </div>
                ...
                <div className="space-y-1.5">
                  <h4 className="text-xs sub-nav-label text-brand-maroon"><LocalizedText text={"CITY SUMMARY"} /></h4>
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <h4 className="text-xs sub-nav-label text-brand-maroon"><LocalizedText text={"CATEGORY RATING MATRIX (0 - 5 STARS)"} /></h4>
                    <span className="text-[11px] text-brand-brown font-mono font-semibold"><LocalizedText text={"Scale: 0.0 – 5.0"} /></span>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-3 text-xs">
                    ...
                    <div className="bg-slate-50 p-2.5 rounded border border-slate-200 space-y-1">
                      <div className="flex justify-between font-semibold text-slate-700">
                        <span><LocalizedText text={"🗺️ Tourism"} /></span>
                    ...
                    <div className="bg-slate-50 p-2.5 rounded border border-slate-200 space-y-1">
                      <div className="flex justify-between font-semibold text-slate-700">
                        <span><LocalizedText text={"🏥 Hospitals"} /></span>
                    ...
                    <div className="bg-slate-50 p-2.5 rounded border border-slate-200 space-y-1">
                      <div className="flex justify-between font-semibold text-slate-700">
                        <span><LocalizedText text={"🏨 Hotels & Stays"} /></span>
                    ...
                    <div className="bg-slate-50 p-2.5 rounded border border-slate-200 space-y-1">
                      <div className="flex justify-between font-semibold text-slate-700">
                        <span><LocalizedText text={"💼 Business & Trade"} /></span>
                    ...
                    <div className="bg-slate-50 p-2.5 rounded border border-slate-200 space-y-1">
                      <div className="flex justify-between font-semibold text-slate-700">
                        <span><LocalizedText text={"🎓 Education"} /></span>
                    ...
                    <div className="bg-slate-50 p-2.5 rounded border border-slate-200 space-y-1">
                      <div className="flex justify-between font-semibold text-slate-700">
                        <span><LocalizedText text={"🏗️ Transport & Infra"} /></span>
                ...
                <div className="space-y-3 pt-2">
                  <div className="flex items-center justify-between">
                    <h4 className="text-xs sub-nav-label text-brand-maroon"><LocalizedText text={" LOCAL PLACES & FACILITIES ("} />{
            ...
            <div className="lg:col-span-12 space-y-4 pt-4 border-t border-brand-brown/15">
              <div className="flex items-center justify-between">
                <h3 className="sub-nav-label text-brand-maroon"><LocalizedText text={"ALL 8 BIHAR CITY HUBS SUMMARY MATRIX"} /></h3>
                <span className="text-xs text-brand-brown font-serif"><LocalizedText text={"Click any hub card or map pin to inspect rating details"} /></span>
              </div>
                ...
                {hubs.map(hub => (
                  ...
                  >
                    ...
                    <div className="pt-2 border-t border-slate-100 flex items-center justify-between text-xs text-brand-maroon font-semibold">
                      <span>{hub.touristPlaces.length}<LocalizedText text={" Places & Services"} /></span>
