...
export const CalendarPage: React.FC = () => {
  ...
  return (
    <div className="pt-28 pb-24 px-4 sm:px-6 md:px-12 max-w-7xl mx-auto my-6 bg-cream/85 backdrop-blur-sm rounded-2xl border border-brand-brown/15 p-4 sm:p-6 md:p-10 shadow-lg space-y-8">
      ...
      {/* Event Grid */}
      {loading ? (
        <div className="py-20 text-center text-brand-brown font-serif text-lg"><LocalizedText text={"Loading 2026 Bihar tourism events..."} /></div>
      ) : events.length === 0 ? (
        <div className="py-20 text-center bg-cream rounded-lg border border-brand-brown/15 p-8 space-y-3">
          <h3 className="font-serif text-2xl text-brand-black font-bold"><LocalizedText text={"No Events Found"} /></h3>
          <p className="text-sm font-serif text-brand-brown/80"><LocalizedText text={" No events scheduled for "} />{viewMode === 'MONTH' ? format(currentMonth, 'MMMM yyyy') : '2026'}<LocalizedText text={" under \""} />{selectedCategory}".
          </p>
          <button
            onClick={() => {
          ...
          ><LocalizedText text={" VIEW ALL 2026 EVENTS "} /></button>
        </div>
      ) : (
        <div className="space-y-4">
          <div className="flex items-center justify-between text-xs font-sans text-brand-brown border-b border-brand-brown/10 pb-2">
            <span><LocalizedText text={"Showing "} /><strong className="text-brand-black">{events.length}</strong><LocalizedText text={" verified event(s) "} />{viewMode === 'MONTH' ? `for ${format(currentMonth, 'MMMM yyyy')}` : 'in 2026'}</span>
          </div>
