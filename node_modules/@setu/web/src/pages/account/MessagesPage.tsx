  ...
  return (
    <div className="pt-28 pb-24 px-4 sm:px-6 md:px-12 max-w-7xl mx-auto space-y-6 font-sans">
      <div className="border-b border-brand-brown/15 pb-4">
        <span className="sub-nav-label text-brand-maroon"><LocalizedText text={"MY ACCOUNT"} /></span>
        <h1 className="text-3xl font-serif text-brand-black font-bold"><LocalizedText text={"My Vendor Messages & Inquiries"} /></h1>
      </div>

      {loading ? (
        <div className="py-20 text-center text-brand-brown font-serif"><LocalizedText text={"Loading your messages..."} /></div>
        ...
        <div className="bg-cream/80 p-12 rounded-xl border border-brand-brown/15 text-center space-y-3">
          ...
          <h3 className="font-serif text-xl text-brand-black"><LocalizedText text={"No Messages Yet"} /></h3>
          <p className="text-sm font-serif text-brand-brown/80"><LocalizedText text={" When you contact local hosts or message vendors about tour offerings, your conversations will appear here. "} /></p>
        </div>
      ) : (
        <div className="bg-white rounded-2xl border border-brand-brown/20 shadow-lg grid grid-cols-1 lg:grid-cols-12 min-h-[550px] overflow-hidden">
          ...
          <div className={`lg:col-span-4 border-r border-brand-brown/15 bg-cream/50 flex flex-col ${
            ...
            <div className="p-4 border-b border-brand-brown/10 font-serif font-bold text-brand-black text-sm sub-nav-label"><LocalizedText text={" VENDOR CONVERSATIONS ("} />{conversations.length})
            </div>
            ...
            {activeConvDetails ? (
              <>
                ...
                <div className="flex-grow p-5 overflow-y-auto space-y-4 max-h-[420px]">
                  {loadingThread ? (
                    <div className="py-12 text-center text-xs text-brand-brown"><LocalizedText text={"Loading thread..."} /></div>
                  ) : messages.length === 0 ? (
                    <div className="py-12 text-center text-xs text-brand-brown"><LocalizedText text={"No messages in this conversation yet."} /></div>
                  ) : (
                    messages.map((msg) => {
                ...
                <form onSubmit={handleSendMessage} className="p-4 border-t border-brand-brown/15 bg-cream/20 flex items-center space-x-3 shrink-0">
                  ...
                  >
                    ...
                    <span className="hidden sm:inline"><LocalizedText text={"SEND"} /></span>
                  </button>
                </form>
              </>
            ) : (
              <div className="flex-grow flex items-center justify-center text-center p-8 text-brand-brown font-serif text-sm"><LocalizedText text={" Select a vendor conversation from the left pane to view messages. "} /></div>
            )}
          </div>
        </div>
      )}
