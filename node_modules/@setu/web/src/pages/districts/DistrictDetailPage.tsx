...
export const DistrictDetailPage: React.FC = () => {
  ...
  if (loading) {
    return <div className="pt-32 pb-24 text-center text-brand-brown font-serif"><LocalizedText text={"Loading district details..."} /></div>;
  }

  if (!district) {
    return (
      <div className="pt-32 pb-24 text-center space-y-4">
        <h2 className="text-3xl font-serif text-brand-black"><LocalizedText text={"District Not Found"} /></h2>
        <Link to="/explore/districts" className="text-brand-maroon underline sub-nav-label"><LocalizedText text={"Return to Districts"} /></Link>
      </div>
    );
  }
    ...
    <div className="pt-24 pb-24">
      ...
      <div className="relative h-[55vh] min-h-[380px] w-full bg-brand-black text-white overflow-hidden">
        ...
        <div className="absolute bottom-10 left-0 right-0 max-w-7xl mx-auto px-6 md:px-12 space-y-3">
          <Link to="/explore/districts" className="inline-flex items-center space-x-2 text-xs sub-nav-label text-brand-gold hover:text-white mb-2 transition-colors">
            ...
            <span><LocalizedText text={"ALL DISTRICTS"} /></span>
          </Link>
          <span className="text-xs sub-nav-label text-brand-gold block">{district.region}</span>
          <h1 className="text-4xl md:text-6xl font-serif text-cream">{district.name}<LocalizedText text={" District"} /></h1>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 pt-12 space-y-12">
        ...
        <div className="bg-cream p-8 rounded border border-brand-brown/15 space-y-4">
          <h3 className="sub-nav-label text-brand-maroon"><LocalizedText text={"DISTRICT OVERVIEW"} /></h3>
        ...
        {galleryImages.length > 0 && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="sub-nav-label text-brand-maroon"><LocalizedText text={"PHOTOGRAPHIC GALLERY OF "} />{district.name.toUpperCase()}</h3>
        ...
        <div className="space-y-6">
          <h3 className="sub-nav-label text-brand-maroon"><LocalizedText text={"DESTINATIONS IN "} />{district.name.toUpperCase()}</h3>
          {district.destinations && district.destinations.length > 0 ? (
            ...
            <p className="text-sm font-serif text-brand-brown/70"><LocalizedText text={"No specific destinations logged for this district yet."} /></p>
          )}
        </div>

        {/* Map */}
        <div className="space-y-4">
          <h3 className="sub-nav-label text-brand-maroon">{district.name.toUpperCase()}<LocalizedText text={" GEOGRAPHIC MAP"} /></h3>
          <InteractiveMap
