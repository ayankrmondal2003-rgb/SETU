  ...
  if (!user) {
    return (
      <div className="pt-32 pb-24 text-center space-y-4">
        <h2 className="text-3xl font-serif text-brand-black"><LocalizedText text={"Please Sign In"} /></h2>
        <p className="text-sm font-serif text-brand-brown"><LocalizedText text={"You need to be authenticated to view your account dashboard."} /></p>
      </div>
    );
  }
  ...
  return (
    <div className="pt-28 pb-24 px-4 sm:px-6 md:px-12 max-w-7xl mx-auto my-6 bg-cream/85 backdrop-blur-sm rounded-2xl border border-brand-brown/15 p-4 sm:p-6 md:p-10 shadow-lg space-y-8">
      ...
      <div className="bg-cream p-5 sm:p-8 rounded border border-brand-brown/15 flex flex-col md:flex-row items-center justify-between gap-6 shadow-sm">
        <div className="flex items-center space-x-5">
          ...
          <div>
            ...
            <p className="text-xs font-sans text-brand-brown/80">{user.email}<LocalizedText text={" • Role: "} /><span className="font-semibold text-brand-maroon">{user.role}</span></p>
          </div>
        </div>
      ...
      <div className="flex flex-wrap border-b border-brand-brown/15 gap-4 sm:space-x-8">
        ...
        ><LocalizedText text={" MY BOOKINGS ("} />{orders.length})
        </button>
        <button
        ...
        ><LocalizedText text={" SAVED FAVORITES ("} />{favorites.length})
        </button>
        <button
        ...
        ><LocalizedText text={" PROFILE SETTINGS "} /></button>
        <button
        ...
        >
          ...
          <span><LocalizedText text={"SETU PLUS"} /></span>
        </button>
      </div>
      ...
      <div>
        {activeTab === 'bookings' && (
          <div className="space-y-4">
            {loading ? (
              <p className="font-serif text-brand-brown text-center py-10"><LocalizedText text={"Loading bookings..."} /></p>
            ) : orders.length === 0 ? (
              <div className="bg-white p-12 text-center rounded border border-brand-brown/15 space-y-3">
                ...
                <h3 className="font-serif text-xl text-brand-black"><LocalizedText text={"No Active Bookings"} /></h3>
                <p className="text-xs font-serif text-brand-brown/70"><LocalizedText text={"Explore tourist offerings and book guided Bihar tours."} /></p>
              </div>
            ) : (
              <div className="space-y-4">
                {orders.map((order) => (
                  <div key={order.id} className="bg-white p-6 rounded border border-brand-brown/15 flex flex-col md:flex-row items-center justify-between gap-6 shadow-sm">
                    <div className="space-y-2">
                      ...
                      <p className="text-xs font-sans text-brand-brown"><LocalizedText text={"Vendor: "} />{order.vendor?.businessName || 'Setu Heritage'}<LocalizedText text={" • Date: "} />{new Date(order.bookingDate).toLocaleDateString()}</p>
                    </div>

                    <div className="text-right space-y-1">
                      <span className="font-serif text-2xl font-bold text-brand-black">₹{order.amount.toLocaleString('en-IN')}</span>
                      <span className="block text-[10px] sub-nav-label text-brand-brown/70">{order.quantity}<LocalizedText text={" Guest(s)"} /></span>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {favorites.length === 0 ? (
              <div className="col-span-full bg-white p-12 text-center rounded border border-brand-brown/15 space-y-2">
                ...
                <p className="text-sm font-serif text-brand-brown"><LocalizedText text={"No saved destinations or events in your favorites list yet."} /></p>
              </div>
            ) : (
              favorites.map((fav) => {
                ...
                return (
                  <div key={fav.id} className="bg-white p-5 rounded-xl border border-brand-brown/15 shadow-sm space-y-3 flex flex-col justify-between hover:shadow-md transition-shadow">
                    <div>
                      <div className="flex items-center justify-between gap-2 mb-2">
                        <span className={`text-[10px] sub-nav-label px-2.5 py-0.5 rounded font-bold uppercase tracking-wider ${
                          ...
                          <LocalizedText text={isEvent ? '🎪 EVENT / FESTIVAL' : '🏛️ DESTINATION'} />
                        </span>
                    ...
                    <div className="pt-2 border-t border-brand-brown/10 flex items-center justify-between">
                      ...
                      >
                        <span><LocalizedText text={"VIEW "} /><LocalizedText text={isEvent ? 'EVENT' : 'DESTINATION'} /></span>
            ...
            {user.isPremium ? (
              <>
                <div className="flex items-center space-x-2 text-brand-maroon">
                  ...
                  <h3 className="sub-nav-label"><LocalizedText text={"SETU PLUS — ACTIVE"} /></h3>
                </div>
                <p className="text-sm font-serif text-brand-black/80"><LocalizedText text={" You have full access to the SETU AI Companion, including multi-day itineraries, local insider tips, and premium experience recommendations. "} /></p>
              </>
            ) : (
              <>
                <div className="flex items-center space-x-2 text-brand-black">
                  <Sparkles className="w-5 h-5 text-brand-gold" />
                  <h3 className="sub-nav-label"><LocalizedText text={"UPGRADE TO SETU PLUS"} /></h3>
                </div>
                <p className="text-sm font-serif text-brand-black/80"><LocalizedText text={" Unlock the full SETU AI Companion — multi-day personalized itineraries, local insider tips, food recommendations, and smart circuit planning. "} /></p>
                <div className="flex items-baseline space-x-2">
                  <span className="font-serif text-3xl font-bold text-brand-black">₹99</span>
                  <span className="text-xs font-sans text-brand-brown"><LocalizedText text={"/ month"} /></span>
                </div>
                <button
                ...
                >
                  <LocalizedText text={subscribing ? 'PROCESSING...' : 'SUBSCRIBE WITH RAZORPAY'} />
                </button>
              </>
            )}
          </div>
        ...
        {activeTab === 'profile' && (
          <div className="bg-white p-8 rounded border border-brand-brown/15 max-w-xl space-y-6">
            <h3 className="sub-nav-label text-brand-maroon"><LocalizedText text={"PERSONAL INFORMATION"} /></h3>
            <div className="space-y-4 text-sm font-sans">
              <div>
                <label className="block text-xs sub-nav-label text-brand-black/70 mb-1"><LocalizedText text={"FULL NAME"} /></label>
              ...
              <div>
                <label className="block text-xs sub-nav-label text-brand-black/70 mb-1"><LocalizedText text={"EMAIL ADDRESS"} /></label>
              ...
              <div>
                <label className="block text-xs sub-nav-label text-brand-black/70 mb-1"><LocalizedText text={"PHONE NUMBER"} /></label>
