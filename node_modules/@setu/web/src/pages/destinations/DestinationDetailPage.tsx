  ...
  if (loading) {
    return (
      <div className="pt-32 pb-24 text-center font-serif space-y-3">
        ...
        <p className="text-brand-brown text-sm"><LocalizedText text={"Loading rich travel guide..."} /></p>
      </div>
    );
  }
  ...
  if (!destination) {
    return (
      <div className="pt-32 pb-24 text-center space-y-4">
        <h2 className="text-3xl font-serif text-brand-black"><LocalizedText text={"Destination Guide Not Found"} /></h2>
        <Link to="/explore/destinations" className="text-brand-maroon underline sub-nav-label"><LocalizedText text={"Return to All Destinations"} /></Link>
      </div>
    );
  }
        ...
        <div className="space-y-8 font-serif leading-relaxed text-brand-black/90">
          ...
          <div className="bg-amber-900 text-cream p-6 md:p-8 rounded-xl border-2 border-brand-gold shadow-lg space-y-3">
            <div className="flex items-center space-x-3 text-brand-gold font-bold text-xs sub-nav-label tracking-widest">
              ...
              <span><LocalizedText text={"DID YOU KNOW? / क्या आप जानते हैं?"} /></span>
            </div>
          ...
          <div className="bg-cream p-6 rounded-xl border border-brand-brown/15 space-y-4">
            <h4 className="sub-nav-label text-brand-maroon text-xs tracking-wider"><LocalizedText text={"HERITAGE HIGHLIGHTS & ARCHITECTURE"} /></h4>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 font-sans text-xs">
              <div className="bg-white p-4 rounded-lg border border-brand-brown/10 shadow-sm space-y-1.5">
                ...
                <p className="text-brand-black font-medium leading-normal"><LocalizedText text={" UNESCO & Archaeological Survey of India (ASI) protected heritage monument. "} /></p>
              </div>
              <div className="bg-white p-4 rounded-lg border border-brand-brown/10 shadow-sm space-y-1.5">
                <span className="w-6 h-6 rounded-full bg-brand-maroon text-white flex items-center justify-center font-bold text-[10px]">
                  2
                </span>
                <p className="text-brand-black font-medium leading-normal"><LocalizedText text={" Authentic ancient brick and stone masonry reflecting Mauryan & Gupta period craft. "} /></p>
              </div>
              <div className="bg-white p-4 rounded-lg border border-brand-brown/10 shadow-sm space-y-1.5">
                <span className="w-6 h-6 rounded-full bg-brand-maroon text-white flex items-center justify-center font-bold text-[10px]">
                  3
                </span>
                <p className="text-brand-black font-medium leading-normal"><LocalizedText text={" Guided pilgrimage pathways with certified local heritage scholars available on site. "} /></p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white p-6 rounded-xl border border-brand-brown/15 space-y-4">
            <h4 className="sub-nav-label text-brand-maroon text-xs"><LocalizedText text={"SITE SPECIFICATIONS"} /></h4>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-xs font-sans">
              ...
              <div>
                <span className="text-brand-brown/70 block text-[10px] sub-nav-label"><LocalizedText text={"CIRCUIT TRAIL"} /></span>
      ...
      content: (
        <div className="space-y-4">
          ...
          {galleryImages.length > 0 ? (
            ...
            <div className="bg-white p-8 rounded-xl border border-brand-brown/15 text-center text-brand-brown/70 font-serif"><LocalizedText text={" No photo gallery images available for this destination. "} /></div>
          )}
        </div>
      )
    },
    {
      ...
      content: (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {/* Stays Column */}
          <div className="space-y-4">
            <h4 className="sub-nav-label text-brand-maroon text-xs"><LocalizedText text={"RECOMMENDED STAYS NEAR SITE"} /></h4>
            <div className="space-y-3">
              {destination.stays && destination.stays.length > 0 ? (
                ...
                <p className="text-xs font-serif text-brand-brown/70"><LocalizedText text={"Check interactive map tab for certified nearby hotel listings."} /></p>
              )}
            </div>
          </div>
            ...
            <div className="bg-cream p-6 rounded-xl border border-brand-brown/15 space-y-3">
              <ul className="space-y-3 font-serif text-sm text-brand-black/85">
                {destination.recommendations && destination.recommendations.length > 0 ? (
                  ...
                  <li><LocalizedText text={"No specific recommendations listed."} /></li>
                )}
              </ul>
            </div>
          </div>
      ...
      <div className="relative h-[65vh] min-h-[440px] w-full bg-brand-black text-white overflow-hidden">
        ...
        <div className="absolute bottom-10 left-0 right-0 max-w-7xl mx-auto px-6 md:px-12 space-y-4">
          ...
          >
            ...
            <span><LocalizedText text={"ALL DESTINATIONS"} /></span>
          </Link>
          ...
          <div className="flex items-center space-x-3 flex-wrap gap-y-2">
            ...
            {isVerified ? (
              <span className="text-xs sub-nav-label bg-emerald-800 text-white px-3 py-1 rounded-full font-bold flex items-center space-x-1">
                ...
                <span><LocalizedText text={"✓ VERIFIED HERITAGE GUIDE"} /></span>
              </span>
            ) : (
              <span className="text-xs sub-nav-label bg-amber-400 text-amber-950 px-3 py-1 rounded-full font-bold flex items-center space-x-1">
                ...
                <span><LocalizedText text={"[NEEDS CONTENT REVIEW]"} /></span>
              </span>
            )}
          </div>
          ...
          <div className="flex items-center space-x-4 text-xs font-sans text-cream/90 pt-1 flex-wrap gap-y-1">
            <div className="flex items-center space-x-1">
              ...
              <span className="font-bold">{destination.district?.name || 'Bihar'}<LocalizedText text={" District"} /></span>
            </div>
            {destination.circuit && (
              <span><LocalizedText text={"• "} />{destination.circuit.name}</span>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 -mt-10 relative z-20">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="bg-white p-5 rounded-xl border border-brand-brown/15 shadow-lg flex items-start space-x-4">
            ...
            <div>
              <span className="text-[10px] sub-nav-label text-brand-brown/70 block"><LocalizedText text={"ENTRY & TIMINGS"} /></span>
          ...
          <div className="bg-white p-5 rounded-xl border border-brand-brown/15 shadow-lg flex items-start space-x-4">
            ...
            <div>
              <span className="text-[10px] sub-nav-label text-brand-brown/70 block"><LocalizedText text={"HOW TO REACH"} /></span>
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 pt-12 space-y-12">
        ...
        {nearbyAttractions.length > 0 && (
          <div className="border-t border-brand-brown/15 pt-12 space-y-6">
            <div className="flex items-center justify-between">
              <div>
                <span className="sub-nav-label text-brand-maroon text-xs"><LocalizedText text={"CROSS-LINKED HERITAGE SPOTS"} /></span>
                <h3 className="text-2xl font-serif text-brand-black font-bold"><LocalizedText text={" Explore Nearby Attractions in "} />{destination.district?.name || 'Region'}
                </h3>
              </div>
