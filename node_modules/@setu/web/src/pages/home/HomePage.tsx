    ...
    <div className="w-full">
      ...
      <section className="relative min-h-screen flex items-center justify-center overflow-hidden bg-brand-black text-white">
        ...
        <div className="absolute bottom-6 left-0 right-0 z-10 px-6 md:px-12 flex items-center justify-between text-xs text-cream/80 sub-nav-label max-w-7xl mx-auto">
          ...
          <span className="hidden md:inline text-white/60"><LocalizedText text={"UNESCO WORLD HERITAGE & MARKETPLACE"} /></span>
        </div>
      </section>
      ...
      <section className="py-24 px-6 md:px-12 bg-cream/85 backdrop-blur-sm text-brand-black border-b border-brand-brown/15">
        <div className="max-w-5xl mx-auto text-center space-y-6">
          ...
          <h2 className="text-3xl md:text-5xl font-serif leading-tight"><LocalizedText text={" Where Empires Were Born and Philosophy Illuminated the World "} /></h2>
          <p className="font-serif text-lg text-brand-black/80 leading-relaxed font-light"><LocalizedText text={" Bihar stands as a timeless cradle of civilization. From the venerable Bodhi Tree at Bodh Gaya to the ancient halls of Nalanda University and the majestic ghats of Patna Sahib, SETU curates extraordinary travel journeys across the state. "} /></p>
        </div>
      </section>
      ...
      <section ref={sectionRef} className="py-24 px-6 md:px-12 max-w-7xl mx-auto my-6 bg-white/85 backdrop-blur-sm rounded-2xl border border-brand-brown/15 shadow-md">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12">
          <div>
            <span className="sub-nav-label text-brand-maroon"><LocalizedText text={"SACRED TRAILS"} /></span>
            <h2 className="text-4xl font-serif text-brand-black mt-2"><LocalizedText text={"Featured Tourism Circuits"} /></h2>
          </div>
          <Link
            to="/explore/circuits"
            className="mt-4 md:mt-0 sub-nav-label text-xs text-brand-maroon hover:text-brand-black flex items-center space-x-1"
          >
            <span><LocalizedText text={"VIEW ALL CIRCUITS"} /></span>
            <ArrowRight className="w-4 h-4" />
          </Link>
        </div>
      ...
      <section className="py-24 px-6 md:px-12 bg-cream-light/85 backdrop-blur-sm border-y border-brand-brown/15">
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-12">
            <div>
              <span className="sub-nav-label text-brand-maroon"><LocalizedText text={"HERITAGE SANCTUARIES"} /></span>
              <h2 className="text-4xl font-serif text-brand-black mt-2"><LocalizedText text={"Iconic Destinations"} /></h2>
            </div>
            <Link
              to="/explore/destinations"
              className="mt-4 md:mt-0 sub-nav-label text-xs text-brand-maroon hover:text-brand-black flex items-center space-x-1"
            >
              <span><LocalizedText text={"EXPLORE ALL DESTINATIONS"} /></span>
              <ArrowRight className="w-4 h-4" />
            </Link>
          </div>
      ...
      <section className="py-24 px-6 md:px-12 max-w-7xl mx-auto my-6 bg-white/85 backdrop-blur-sm rounded-2xl border border-brand-brown/15 shadow-md">
        <div className="text-center max-w-3xl mx-auto mb-12 space-y-3">
          <span className="sub-nav-label text-brand-maroon"><LocalizedText text={"GEOGRAPHIC DISCOVERY"} /></span>
          <h2 className="text-4xl font-serif text-brand-black"><LocalizedText text={"Interactive Bihar Tourism Map"} /></h2>
          <p className="font-serif text-brand-black/75"><LocalizedText text={" Filter markers by spiritual circuit, district, or heritage category. Click pins to explore detail cards. "} /></p>
        </div>
      ...
      <section className="py-24 px-6 md:px-12 bg-cream/85 backdrop-blur-sm border-t border-brand-brown/15">
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-12">
            <div>
              <span className="sub-nav-label text-brand-maroon"><LocalizedText text={"FESTIVALS & FAIRS"} /></span>
              <h2 className="text-4xl font-serif text-brand-black mt-2"><LocalizedText text={"Upcoming Tourism Events"} /></h2>
            </div>
            <Link
              to="/calendar"
              className="mt-4 md:mt-0 sub-nav-label text-xs text-brand-maroon hover:text-brand-black flex items-center space-x-1"
            >
              <span><LocalizedText text={"VIEW FULL 2026 CALENDAR"} /></span>
              <ArrowRight className="w-4 h-4" />
            </Link>
          </div>
          ...
          {upcomingEvents.length > 0 ? (
            ...
            <div className="bg-white/80 backdrop-blur-sm p-10 rounded-xl border border-brand-brown/15 text-center space-y-3 shadow-sm">
              ...
              <h3 className="font-serif text-xl font-medium text-brand-black"><LocalizedText text={"No Upcoming Events Right Now"} /></h3>
              <p className="font-serif text-brand-black/75 text-sm max-w-md mx-auto"><LocalizedText text={" There are currently no scheduled upcoming festival dates remaining for this period. Check back soon or view the full 2026 calendar archive. "} /></p>
              <div className="pt-2">
                <Link
                  to="/calendar"
                ...
                >
                  <span><LocalizedText text={"BROWSE FULL 2026 CALENDAR"} /></span>
                  <ArrowRight className="w-4 h-4" />
                </Link>
              </div>
            </div>
      ...
      <section className="py-20 px-6 md:px-12 bg-brand-black/90 backdrop-blur-md text-cream">
        <div className="max-w-5xl mx-auto text-center space-y-6">
          <div className="inline-flex items-center space-x-2 px-3 py-1 bg-brand-gold/20 text-brand-gold rounded-full text-xs sub-nav-label">
            ...
            <span><LocalizedText text={"INTELLIGENT TRAVEL COMPANION"} /></span>
          </div>
          <h2 className="text-3xl md:text-5xl font-serif text-cream"><LocalizedText text={"Plan Your Bespoke Bihar Journey with AI"} /></h2>
          <p className="font-serif text-cream/70 text-base max-w-2xl mx-auto leading-relaxed"><LocalizedText text={" Curate personalized day-by-day itineraries tailored to your dates, spiritual interests, and regional culinary preferences. "} /></p>
        </div>
      </section>
    </div>
  );
