  ...
  if (!user || user.role !== 'VENDOR') {
    return (
      <div className="pt-32 pb-24 text-center space-y-4">
        <h2 className="text-3xl font-serif text-brand-black"><LocalizedText text={"Vendor Access Required"} /></h2>
        <p className="text-sm font-serif text-brand-brown"><LocalizedText text={"Please log in as a local vendor to access this simple dashboard."} /></p>
      </div>
    );
  }
    ...
    <div className="pt-28 pb-24 px-4 sm:px-8 max-w-6xl mx-auto my-6 bg-cream/85 backdrop-blur-sm rounded-2xl border border-brand-brown/15 p-6 sm:p-10 shadow-lg space-y-8">
      ...
      <div className="bg-amber-900 text-cream p-6 sm:p-8 rounded-xl shadow-lg border-2 border-brand-gold flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
        <div className="flex items-center space-x-4">
          ...
          <div className="space-y-1">
            ...
            <p className="text-sm font-sans text-brand-gold/90 font-medium"><LocalizedText text={" Category: "} />{vendor?.businessType || 'Local Crafts & Goods'}<LocalizedText text={" • Location: "} />{vendor?.city || 'Bihar'}, {vendor?.district || ''}
            </p>
          </div>
        </div>
      ...
      {vendor?.status === 'PENDING' && (
        <div className="bg-amber-100 border-2 border-amber-400 p-5 rounded-lg text-amber-950 text-sm font-sans flex items-center space-x-4">
          ...
          <div>
            <p className="font-bold text-base"><LocalizedText text={"Your listing is being reviewed by the SETU team."} /></p>
            <p className="text-xs text-amber-900 mt-0.5"><LocalizedText text={"You can add your products now so customers can buy them as soon as you are approved!"} /></p>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
        ...
        >
          ...
          <div>
            ...
            <span className="text-xs opacity-90 font-sans"><LocalizedText text={"List Madhubani art, Makhana, Silk, or local items"} /></span>
          </div>
        </button>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
        <div className="bg-white p-6 rounded-xl border-2 border-brand-brown/15 shadow-sm text-center space-y-1">
          <span className="text-xs font-bold text-brand-brown uppercase tracking-wider"><LocalizedText text={"PRODUCTS LISTED"} /></span>
          <p className="font-serif text-4xl font-extrabold text-brand-black">{offerings.length}</p>
        </div>

        <div className="bg-white p-6 rounded-xl border-2 border-brand-brown/15 shadow-sm text-center space-y-1">
          <span className="text-xs font-bold text-brand-brown uppercase tracking-wider"><LocalizedText text={"CUSTOMER ORDERS"} /></span>
          <p className="font-serif text-4xl font-extrabold text-brand-maroon">{orders.length}</p>
        </div>

        <div className="bg-white p-6 rounded-xl border-2 border-brand-brown/15 shadow-sm text-center space-y-1">
          <span className="text-xs font-bold text-brand-brown uppercase tracking-wider"><LocalizedText text={"TOTAL SALES"} /></span>
      ...
      <div className="bg-white p-6 sm:p-8 rounded-xl border-2 border-brand-brown/15 space-y-6">
        <div className="flex items-center justify-between border-b-2 border-brand-brown/10 pb-4">
          <h2 className="text-2xl font-serif font-bold text-brand-black flex items-center space-x-2">
            ...
            <span><LocalizedText text={"YOUR PRODUCTS CATALOG"} /></span>
          </h2>
          <span className="text-sm font-bold text-brand-maroon">{offerings.length}<LocalizedText text={" Product(s)"} /></span>
        </div>

        {offerings.length === 0 ? (
          <div className="text-center py-10 space-y-4">
            <p className="text-base font-serif text-brand-brown"><LocalizedText text={"You have not added any products yet."} /></p>
            <button
              onClick={() => setShowAddProductModal(true)}
              className="px-6 py-3 bg-emerald-700 text-white font-bold rounded-lg text-sm hover:bg-emerald-800"
            ><LocalizedText text={" + Add First Product Now "} /></button>
          </div>
        ) : (
            ...
            {offerings.map((product) => (
              <div key={product.id} className="border-2 border-brand-brown/15 rounded-xl overflow-hidden bg-cream-light p-4 space-y-3 shadow-sm">
                ...
                <div className="flex items-center justify-between pt-3 border-t border-brand-brown/10">
                  ...
                  <span className="text-xs font-bold text-emerald-800 bg-emerald-100 px-2.5 py-1 rounded"><LocalizedText text={"Active"} /></span>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="bg-white p-6 sm:p-8 rounded-xl border-2 border-brand-brown/15 space-y-6">
        <h2 className="text-2xl font-serif font-bold text-brand-black flex items-center space-x-2">
          ...
          <span><LocalizedText text={"RECENT CUSTOMER ORDERS"} /></span>
        </h2>

        {orders.length === 0 ? (
          <p className="text-sm font-serif text-brand-brown/70 text-center py-6"><LocalizedText text={"No customer orders received yet."} /></p>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-left font-sans text-sm border-collapse">
              <thead>
                <tr className="border-b-2 border-brand-brown/15 text-xs font-bold text-brand-brown uppercase">
                  <th className="py-3 px-3"><LocalizedText text={"ORDER #"} /></th>
                  <th className="py-3 px-3"><LocalizedText text={"BUYER NAME"} /></th>
                  <th className="py-3 px-3"><LocalizedText text={"PRODUCT"} /></th>
                  <th className="py-3 px-3"><LocalizedText text={"PRICE"} /></th>
                  <th className="py-3 px-3"><LocalizedText text={"STATUS"} /></th>
                </tr>
              </thead>
      ...
      {showAddProductModal && (
        <div className="fixed inset-0 z-50 bg-black/75 flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-md p-6 sm:p-8 rounded-2xl shadow-2xl space-y-6 max-h-[90vh] overflow-y-auto border-4 border-emerald-600">
            <h3 className="font-serif text-2xl font-bold text-brand-black"><LocalizedText text={"Add New Product or Craft"} /></h3>

            <form onSubmit={handleAddProduct} className="space-y-4 text-sm font-sans">
              <div>
                <label className="block font-bold text-brand-black mb-1"><LocalizedText text={"PRODUCT / CRAFT TITLE"} /></label>
                <input
                  type="text"
                  required
                  value={title}
              ...
              <div>
                <label className="block font-bold text-brand-black mb-1"><LocalizedText text={"SHORT DESCRIPTION"} /></label>
                <textarea
                  required
                  rows={3}
                  value={description}
              ...
              <div>
                <label className="block font-bold text-brand-black mb-1"><LocalizedText text={"CATEGORY"} /></label>
                <select
                  value={category}
                  onChange={(e) => setCategory(e.target.value)}
                  className="w-full border-2 border-brand-brown/30 p-3 rounded-lg text-base font-bold bg-cream"
                >
                  <option value="Handicraft"><LocalizedText text={"Handicraft & Art"} /></option>
                  <option value="Silk & Handloom"><LocalizedText text={"Silk & Handloom"} /></option>
                  <option value="Gourmet Food"><LocalizedText text={"Gourmet Food (Makhana, Sattu, Sweets)"} /></option>
                  <option value="Local Products"><LocalizedText text={"Local Souvenirs & Goods"} /></option>
                  <option value="Transport"><LocalizedText text={"Local Transport & Taxi"} /></option>
                </select>
              </div>

              <div>
                <label className="block font-bold text-brand-black mb-1"><LocalizedText text={"PRICE (INR ₹)"} /></label>
                <input
                  type="number"
                  required
                  min={10}
              ...
              <div>
                <label className="block font-bold text-brand-black mb-1"><LocalizedText text={"PHOTO URL"} /></label>
                <input
                  type="url"
                  required
                  value={coverImage}
              ...
              <div className="flex items-center justify-end space-x-3 pt-4 border-t-2">
                ...
                ><LocalizedText text={" SAVE PRODUCT "} /></button>
              </div>
            </form>
          </div>
        </div>
      ...
      {showEditProfileModal && (
        <div className="fixed inset-0 z-50 bg-black/75 flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-md p-6 sm:p-8 rounded-2xl shadow-2xl space-y-6 border-4 border-brand-gold">
            <h3 className="font-serif text-2xl font-bold text-brand-black"><LocalizedText text={"Edit Business Details"} /></h3>

            <form onSubmit={handleSaveProfile} className="space-y-4 text-sm font-sans">
              <div>
                <label className="block font-bold text-brand-black mb-1"><LocalizedText text={"BUSINESS NAME"} /></label>
                <input
                  type="text"
                  required
              ...
              <div>
                <label className="block font-bold text-brand-black mb-1"><LocalizedText text={"PHONE NUMBER"} /></label>
                <input
                  type="text"
                  required
                  value={editPhone}
              ...
              <div>
                <label className="block font-bold text-brand-black mb-1"><LocalizedText text={"ADDRESS / MARKET"} /></label>
                <input
                  type="text"
                  required
                  value={editAddress}
              ...
              <div>
                <label className="block font-bold text-brand-black mb-1"><LocalizedText text={"CITY / DISTRICT"} /></label>
                <input
                  type="text"
                  required
                  value={editCity}
              ...
              <div className="flex items-center justify-end space-x-3 pt-4 border-t-2">
                ...
                ><LocalizedText text={" UPDATE INFO "} /></button>
              </div>
            </form>
          </div>
        </div>
