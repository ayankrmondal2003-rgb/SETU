    ...
    <div className="pt-28 pb-24 px-4 sm:px-6 md:px-12 max-w-4xl mx-auto space-y-8 font-sans">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-brand-brown/15 pb-6">
        <div>
          ...
          <h1 className="text-3xl font-serif text-brand-black font-bold"><LocalizedText text={"Vendor Notifications"} /></h1>
        </div>
        ...
        {notifications.some(n => !n.isRead) && (
          ...
          ><LocalizedText text={" MARK ALL AS READ "} /></button>
        )}
      </div>
      ...
      <div className="flex space-x-3 border-b border-brand-brown/10 pb-2">
        ...
        ><LocalizedText text={" ALL ("} />{notifications.length})
        </button>
        <button
        ...
        ><LocalizedText text={" UNREAD ("} />{notifications.filter(n => !n.isRead).length})
        </button>
      </div>

      {loading ? (
        <div className="py-20 text-center text-brand-brown font-serif"><LocalizedText text={"Loading notifications..."} /></div>
      ) : filtered.length === 0 ? (
        <div className="bg-cream/80 p-12 rounded-xl border border-brand-brown/15 text-center space-y-3">
          ...
          <h3 className="font-serif text-xl text-brand-black"><LocalizedText text={"No notifications found"} /></h3>
          <p className="text-sm font-serif text-brand-brown/80"><LocalizedText text={"You're all caught up! New order and account updates will appear here."} /></p>
        </div>
      ) : (
