  ...
  if (!user || user.role !== 'VENDOR') {
    return (
      <div className="pt-32 pb-24 text-center space-y-4">
        ...
        <p className="text-sm font-serif text-brand-brown"><LocalizedText text={"You must be logged in as a Vendor to view this dashboard."} /></p>
      </div>
    );
  }
    ...
    <div className="pt-28 pb-24 px-4 sm:px-6 md:px-12 max-w-7xl mx-auto my-6 bg-cream/85 backdrop-blur-sm rounded-2xl border border-brand-brown/15 p-4 sm:p-6 md:p-10 shadow-lg space-y-8">
      ...
      <div className="bg-cream p-4 sm:p-8 rounded border border-brand-brown/15 flex flex-col md:flex-row items-center justify-between gap-6 shadow-sm">
        <div className="flex flex-col sm:flex-row items-center space-x-0 sm:space-x-5 text-center sm:text-left gap-3">
          ...
          <div>
            <div className="flex items-center space-x-3">
              ...
              }`}><LocalizedText text={" STATUS: "} />{vendor?.status || 'PENDING'}
              </span>
            </div>
            <p className="text-xs font-sans text-brand-brown/80 mt-1"><LocalizedText text={" Business Type: "} />{vendor?.businessType || 'Tour Operator'}<LocalizedText text={" • "} />{vendor?.city}, {vendor?.district}
            </p>
          </div>
        </div>
        ...
        {vendor?.status === 'APPROVED' && (
          ...
          >
            ...
            <span><LocalizedText text={"CREATE NEW OFFERING"} /></span>
          </button>
        )}
      </div>
      ...
      {vendor?.status === 'PENDING' && (
        <div className="bg-amber-50 border border-amber-300 p-4 rounded text-amber-900 text-xs font-sans flex items-center space-x-3">
          ...
          <span><LocalizedText text={"Your vendor account is pending admin approval. You can prepare offering details, but active publication is restricted until approved."} /></span>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        <div className="bg-white p-6 rounded border border-brand-brown/15 shadow-sm space-y-2">
          <span className="text-[10px] sub-nav-label text-brand-brown/70"><LocalizedText text={"GROSS SALES"} /></span>
          <p className="font-serif text-3xl font-bold text-brand-black">₹{grossSales.toLocaleString('en-IN')}</p>
        </div>

        <div className="bg-white p-6 rounded border border-brand-brown/15 shadow-sm space-y-2">
          <span className="text-[10px] sub-nav-label text-brand-brown/70"><LocalizedText text={"SETU COMMISSION (7%)"} /></span>
          <p className="font-serif text-3xl font-bold text-brand-maroon">₹{setuCommission.toLocaleString('en-IN')}</p>
        </div>

        <div className="bg-white p-6 rounded border border-brand-brown/15 shadow-sm space-y-2">
          <span className="text-[10px] sub-nav-label text-brand-brown/70"><LocalizedText text={"NET EARNINGS"} /></span>
          <p className="font-serif text-3xl font-bold text-emerald-700">₹{netEarnings.toLocaleString('en-IN')}</p>
        </div>

        <div className="bg-white p-6 rounded border border-brand-brown/15 shadow-sm space-y-2">
          <span className="text-[10px] sub-nav-label text-brand-brown/70"><LocalizedText text={"TOTAL ORDERS"} /></span>
          <p className="font-serif text-3xl font-bold text-brand-black">{orders.length}</p>
        </div>

        <div className="bg-white p-6 rounded border border-brand-brown/15 shadow-sm space-y-2">
          <span className="text-[10px] sub-nav-label text-brand-brown/70"><LocalizedText text={"ACTIVE OFFERINGS"} /></span>
          <p className="font-serif text-3xl font-bold text-brand-black">{offerings.length}</p>
        </div>

        <div className="bg-white p-6 rounded border border-brand-brown/15 shadow-sm space-y-2">
          <span className="text-[10px] sub-nav-label text-brand-brown/70"><LocalizedText text={"PENDING BOOKINGS"} /></span>
      ...
      <div className="bg-white p-8 rounded border border-brand-brown/15 space-y-6">
        <div className="flex items-center justify-between border-b border-brand-brown/15 pb-4">
          <h3 className="sub-nav-label text-brand-maroon"><LocalizedText text={"YOUR TOURISM OFFERINGS"} /></h3>
          <span className="text-xs font-sans text-brand-brown">{offerings.length}<LocalizedText text={" Active Listing(s)"} /></span>
        </div>

        {offerings.length === 0 ? (
          <p className="text-sm font-serif text-brand-brown/70 text-center py-6"><LocalizedText text={"No offerings published yet."} /></p>
        ) : (
      ...
      <div className="bg-white p-8 rounded border border-brand-brown/15 space-y-6">
        <h3 className="sub-nav-label text-brand-maroon"><LocalizedText text={"BOOKING ORDERS RECEIVED"} /></h3>

        {orders.length === 0 ? (
          <p className="text-sm font-serif text-brand-brown/70 text-center py-6"><LocalizedText text={"No orders received yet."} /></p>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-left font-sans text-sm border-collapse">
              <thead>
                <tr className="border-b border-brand-brown/15 text-[11px] sub-nav-label text-brand-brown">
                  <th className="py-3 px-2"><LocalizedText text={"ORDER #"} /></th>
                  <th className="py-3 px-2"><LocalizedText text={"CUSTOMER"} /></th>
                  <th className="py-3 px-2"><LocalizedText text={"OFFERING"} /></th>
                  <th className="py-3 px-2"><LocalizedText text={"BOOKING DATE"} /></th>
                  <th className="py-3 px-2"><LocalizedText text={"AMOUNT"} /></th>
                  <th className="py-3 px-2"><LocalizedText text={"PAYMENT"} /></th>
                  <th className="py-3 px-2"><LocalizedText text={"STATUS ACTION"} /></th>
                </tr>
              </thead>
                  ...
                  <tr key={order.id} className="hover:bg-cream-light">
                    ...
                    <td className="py-4 px-2">
                      ...
                      >
                        ...
                        <option value="PROCESSING"><LocalizedText text={"PROCESSING"} /></option>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-lg p-8 rounded shadow-2xl space-y-6 max-h-[90vh] overflow-y-auto">
            <h3 className="font-serif text-2xl text-brand-black"><LocalizedText text={"Create Tourism Offering"} /></h3>

            <form onSubmit={handleCreateOffering} className="space-y-4 font-sans text-xs">
              <div>
                <label className="block sub-nav-label mb-1"><LocalizedText text={"TITLE"} /></label>
              ...
              <div>
                <label className="block sub-nav-label mb-1"><LocalizedText text={"DESCRIPTION"} /></label>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block sub-nav-label mb-1"><LocalizedText text={"CATEGORY"} /></label>
                  <select value={category} onChange={(e) => setCategory(e.target.value)} className="w-full bg-cream border p-2.5 rounded text-sm text-brand-black">
                    <option value="Guided Tour"><LocalizedText text={"Guided Tour"} /></option>
                    <option value="Heritage Walk"><LocalizedText text={"Heritage Walk"} /></option>
                    <option value="Cultural Experience"><LocalizedText text={"Cultural Experience"} /></option>
                    <option value="Stay"><LocalizedText text={"Heritage Stay"} /></option>
                  </select>
                </div>
                <div>
                  <label className="block sub-nav-label mb-1"><LocalizedText text={"PRICE (INR)"} /></label>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block sub-nav-label mb-1"><LocalizedText text={"DURATION"} /></label>
                ...
                <div>
                  <label className="block sub-nav-label mb-1"><LocalizedText text={"LOCATION"} /></label>
              ...
              <div>
                <label className="block sub-nav-label mb-1"><LocalizedText text={"COVER IMAGE URL"} /></label>
              ...
              <div className="flex items-center justify-end space-x-3 pt-4 border-t">
                <button type="button" onClick={() => setShowModal(false)} className="px-4 py-2 border rounded sub-nav-label"><LocalizedText text={"CANCEL"} /></button>
                <button type="submit" className="px-6 py-2 bg-brand-black text-brand-gold rounded sub-nav-label hover:bg-brand-maroon hover:text-white"><LocalizedText text={"CREATE OFFERING"} /></button>
              </div>
            </form>
          </div>
        </div>
